    ...
    <Card className="group flex min-w-0 flex-col overflow-hidden rounded-lg border-slate-200 bg-white shadow-none transition-[transform,box-shadow,border-color] duration-150 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:hover:border-slate-600">
      ...
      <div className="flex flex-1 flex-col p-3">
        <div className="flex-1">
          ...
          {outOfStock ? (
            <span className="mt-2 inline-flex rounded-full bg-red-50 px-2 py-1 text-xs font-semibold text-red-700 ring-1 ring-red-200">Out of Stock</span>
          ) : lowStock ? (
            <span className="mt-2 inline-flex rounded-full bg-amber-50 px-2 py-1 text-xs font-semibold text-amber-700 ring-1 ring-amber-200">Low Stock · {stock} {item.stockUnit || "pcs"}</span>
          ) : tracked ? (
            <span className="mt-2 block text-xs font-medium text-slate-500">{stock} {item.stockUnit || "pcs"} available</span>
          ) : null}
        </div>
        ...
        <Button className="mt-3 h-9 w-full gap-1.5" onClick={onAdd} disabled={outOfStock}>
