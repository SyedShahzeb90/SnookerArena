...
function CanteenProfitReportPage() {
  ...
  const cards = [
    ["Canteen Sales Revenue", money(report.salesRevenue)],
    ...
    <main className="min-h-screen bg-slate-100 px-4 py-6 sm:px-6">
      <div className="mx-auto max-w-7xl space-y-5">
        <header>
          ...
          <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-lg border bg-white"><BarChart3 className="h-5 w-5" /></div><div><h1 className="text-2xl font-bold text-slate-950">Canteen Profit Report</h1><p className="text-sm text-slate-500">Estimate Canteen gross profit from sales and vendor restocking costs.</p></div></div>
        </header>
        ...
        {(report.unallocatedLegacyRevenue > 0 || report.quantityWithoutCost > 0) && <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">{report.unallocatedLegacyRevenue > 0 && <p>{money(report.unallocatedLegacyRevenue)} of legacy Canteen revenue has no product-level line allocation.</p>}{report.quantityWithoutCost > 0 && <p>{report.quantityWithoutCost} sold items have no vendor cost history, so their estimated cost is currently Rs. 0.</p>}</div>}

        <Card className="overflow-hidden">
          <div className="relative border-b p-4"><Search className="absolute left-7 top-7 h-4 w-4 text-slate-400" /><Input className="pl-9" placeholder="Search Canteen product..." value={search} onChange={(event) => setSearch(event.target.value)} /></div>
          <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr>{["Product", "Quantity Sold", "Canteen Revenue", "Average Unit Cost", "Estimated Cost", "Estimated Gross Profit"].map((label) => <th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row.menuItemId} className="border-t bg-white"><td className="px-4 py-3 font-semibold">{row.productName}</td><td className="px-4 py-3">{row.quantitySold} {row.unit}</td><td className="px-4 py-3 font-semibold">{money(row.salesRevenue)}</td><td className="px-4 py-3">{row.averageUnitCost === undefined ? "No cost data" : money(row.averageUnitCost)}</td><td className="px-4 py-3">{money(row.estimatedInventoryCost)}</td><td className={`px-4 py-3 font-bold ${row.estimatedGrossProfit < 0 ? "text-red-700" : "text-emerald-700"}`}>{money(row.estimatedGrossProfit)}</td></tr>)}{rows.length === 0 && <tr><td colSpan={6} className="px-4 py-10 text-center text-slate-500">No Canteen sales found for this period.</td></tr>}</tbody></table></div>
        </Card>
      </div>
    </main>
  );
